...
@section('title', 'إدارة مستويات المخاطر')

@section('content')
<div class="container-fluid px-4">
    <div class="d-flex justify-content-between align-items-center mb-4">
        <h3>إدارة مستويات المخاطر (Risk Levels)</h3>
        <button class="btn-gold" data-bs-toggle="modal" data-bs-target="#addModal">إضافة مستوى</button>
    </div>
    ...
    <div class="table-responsive card-custom">
        <table class="table table-hover mb-0">
            <thead><tr><th>#</th><th>الكود</th><th>العربية</th><th>الإنجليزية</th><th>الحد الأدنى</th><th>الحد الأعلى</th><th>اللون</th><th>إجراءات</th></tr></thead>
            <tbody>
                @forelse($items as $item)
                <tr>
                    <td>{{ $item['risk_level_id'] ?? $loop->iteration }}</td>
                    ...
                    <td>
                        ...
                        <form action="{{ route('admin.dictionaries.risk-levels.destroy', $item['risk_level_id']) }}" method="POST" class="d-inline" onsubmit="return confirm('متأكد من الحذف؟')">
                            @csrf @method('DELETE')
                ...
                @empty
                <tr><td colspan="8" class="text-center py-4">لا توجد بيانات</td></tr>
                @endforelse
            </tbody>
        </table>
    </div>
    ...
    <div class="modal fade" id="addModal" tabindex="-1">
        <div class="modal-dialog"><div class="modal-content">
            <div class="modal-header bg-primary text-white"><h5>إضافة مستوى خطر</h5><button class="btn-close btn-close-white" data-bs-dismiss="modal"></button></div>
            <form method="POST" action="{{ route('admin.dictionaries.risk-levels.store') }}">
                @csrf
                <div class="modal-body">
                    ...
                    <div class="mb-3"><label class="form-label">الحد الأدنى</label><input type="number" name="min_score" class="form-control" required></div>
                    <div class="mb-3"><label class="form-label">الحد الأعلى</label><input type="number" name="max_score" class="form-control" required></div>
    ...
    <div class="modal fade" id="editModal" tabindex="-1">
        <div class="modal-dialog"><div class="modal-content">
            <div class="modal-header bg-warning"><h5>تعديل مستوى خطر</h5><button class="btn-close" data-bs-dismiss="modal"></button></div>
            <form id="editForm" method="POST">
                ...
                <div class="modal-body">
                    ...
                    <div class="mb-3"><label class="form-label">الحد الأدنى</label><input type="number" name="min_score" id="e_min" class="form-control" required></div>
                    <div class="mb-3"><label class="form-label">الحد الأعلى</label><input type="number" name="max_score" id="e_max" class="form-control" required></div>
...
function editItem(item) {
    document.getElementById('editForm').action = '/admin/dictionaries/risk-levels/' + item.risk_level_id;
    ...
    document.getElementById('e_min').value = item.min_score || '';
    document.getElementById('e_max').value = item.max_score || '';
