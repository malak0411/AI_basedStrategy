...
<div class="container-fluid px-4">
    <a href="{{ route('strategic.programs.show', $program['id']) }}" class="btn btn-outline-secondary mb-3">
        <i class="fas fa-arrow-right"></i> العودة
    </a>

    <div class="card-custom">
        <h4 class="mb-4"><i class="fas fa-edit ml-2"></i>تعديل البرنامج</h4>

        <form method="POST" action="{{ route('strategic.programs.update', $program['id']) }}">
            @csrf
            @method('PUT')
            <div class="row">
                ...
                <div class="col-md-4 mb-3">
                    ...
                    <select name="goal_id" class="form-control" required>
                        @foreach($goals as $goal)
                            <option value="{{ $goal['id'] }}" {{ ($program['goal_id'] ?? '') == $goal['id'] ? 'selected' : '' }}>
                ...
                <div class="col-12 mb-3">
                    ...
                    <textarea name="description" class="form-control" rows="4">{{ $program['description'] ?? '' }}</textarea>
                </div>
                <div class="col-md-4 mb-3">
                    <label class="form-label">الميزانية</label>
                    <input type="number" name="budget" class="form-control" step="0.01" value="{{ $program['budget'] ?? '' }}">
                </div>
                <div class="col-md-4 mb-3">
