...
<div class="container-fluid px-4">
    <div class="d-flex justify-content-between align-items-center mb-4">
        <div>
            <h3><i class="fas fa-edit ml-2"></i>تعديل الخطر</h3>
    ...
    <div class="card-custom">
        <div class="card-body">
            <form action="{{ route('risks.update', $risk['risk_id']) }}" method="POST">
                @csrf
                @method('PUT')
                    ...
                    <div class="col-md-8">
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" name="name" class="form-control @error('name') is-invalid @enderror" 
                                value="{{ old('name', $risk['name'] ?? '') }}" required>
                            @error('name')
                        ...
                        <div class="mb-3">
                            ...
                            <textarea name="description" class="form-control @error('description') is-invalid @enderror" 
                                rows="3">{{ old('description', $risk['description'] ?? '') }}</textarea>
                            @error('description')
                            <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                        ...
                        <div class="mb-3">
                            ...
                            <select name="task_id" class="form-control @error('task_id') is-invalid @enderror">
                                ...
                                <option value="{{ $task['task_id'] }}" 
                                    {{ old('task_id', $risk['task_id'] ?? '') == $task['task_id'] ? 'selected' : '' }}>
                                    {{ $task['title'] }}
                                </option>
                                @endforeach
                            </select>
                            ...
                            <div class="col-md-6">
                                <div class="mb-3">
                                    ...
                                    <select name="probability" class="form-control @error('probability') is-invalid @enderror" required>
                                        ...
                                        <option value="1" {{ old('probability', $risk['probability'] ?? '') == 1 ? 'selected' : '' }}>1 - منخفض جداً</option>
                                        <option value="2" {{ old('probability', $risk['probability'] ?? '') == 2 ? 'selected' : '' }}>2 - منخفض</option>
                                        <option value="3" {{ old('probability', $risk['probability'] ?? '') == 3 ? 'selected' : '' }}>3 - متوسط</option>
                                        <option value="4" {{ old('probability', $risk['probability'] ?? '') == 4 ? 'selected' : '' }}>4 - مرتفع</option>
                                        <option value="5" {{ old('probability', $risk['probability'] ?? '') == 5 ? 'selected' : '' }}>5 - مرتفع جداً</option>
                                    </select>
                                    @error('probability')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                    @enderror
                            ...
                            <div class="col-md-6">
                                <div class="mb-3">
                                    ...
                                    <select name="impact" class="form-control @error('impact') is-invalid @enderror" required>
                                        ...
                                        <option value="1" {{ old('impact', $risk['impact'] ?? '') == 1 ? 'selected' : '' }}>1 - ضئيل</option>
                                        <option value="2" {{ old('impact', $risk['impact'] ?? '') == 2 ? 'selected' : '' }}>2 - بسيط</option>
                                        <option value="3" {{ old('impact', $risk['impact'] ?? '') == 3 ? 'selected' : '' }}>3 - متوسط</option>
                                        <option value="4" {{ old('impact', $risk['impact'] ?? '') == 4 ? 'selected' : '' }}>4 - كبير</option>
                                        <option value="5" {{ old('impact', $risk['impact'] ?? '') == 5 ? 'selected' : '' }}>5 - كارثي</option>
                                    </select>
                                    @error('impact')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                    @enderror
                                ...
                                <div class="mb-3">
                                    ...
                                    <select name="status_id" class="form-control @error('status_id') is-invalid @enderror">
                                        ...
                                        <option value="{{ $status['status_id'] }}" 
                                            {{ old('status_id', $risk['status']['status_id'] ?? '') == $status['status_id'] ? 'selected' : '' }}>
                                            {{ $status['name_ar'] }}
                                        </option>
                                        @endforeach
                                    </select>
                                ...
                                <div class="text-center">
                                    <div class="row">
                                        <div class="col-6">
                                            ...
                                            <h4 id="probabilityDisplay" class="text-primary">{{ $risk['probability'] ?? 0 }}</h4>
                                        </div>
                                        <div class="col-6">
                                            <small class="text-muted">التأثير</small>
                                            <h4 id="impactDisplay" class="text-primary">{{ $risk['impact'] ?? 0 }}</h4>
                                        </div>
                                    </div>
                                    <div class="mt-3">
                                        <small class="text-muted">درجة الخطر</small>
                                        <h2 id="riskScoreDisplay" class="fw-bold">{{ $risk['risk_score'] ?? 0 }}</h2>
                                    </div>
                                    <div class="mt-2">
                                        <small class="text-muted">مستوى الخطر</small>
                                        <h4 id="riskLevelDisplay">
                                            @php
                ...
                <div class="border-top pt-3 mt-3">
                    <button type="submit" class="btn btn-primary">
                        <i class="fas fa-save"></i> تحديث الخطر
                    </button>
