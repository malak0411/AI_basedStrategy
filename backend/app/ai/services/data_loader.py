"""
Data Loader Service - Dynamic Version
يكتشف أرقام الحالات تلقائياً من dict_statuses
"""
import pandas as pd
import numpy as np
...
class DataLoader:
    ...
    def __init__(self):
        ...
        self._delayed_id = None  # سيتم اكتشافه تلقائياً
    
    @property
    def delayed_status_id(self):
        """اكتشاف status_id الخاص بالحالة 'متأخر' تلقائياً"""
    ...
    def extract_features(self, tasks):
        """استخراج الميزات الهندسية"""
        print(f"\n🔧 بدء استخراج الميزات لـ {len(tasks)} مهمة...")
        ...
        for i, task in enumerate(tasks):
            ...
            try:
                ...
                print(f"  ⚠️ خطأ في المهمة #{task.task_id}: {str(e)}")
                continue
    ...
    def _extract_single_task(self, task):
        """استخراج جميع الميزات لمهمة واحدة"""
        today = date.today()
        tid = task.task_id
        ...
        # 9. الهدف - ديناميكي من dict_statuses
