...
class DataLoader:
    ...
    def _extract_single_task(self, task):
        ...
        completed_status = self.db.query(DictStatus).filter(
            DictStatus.code.in_(['completed', 'resolved']),
            DictStatus.category.in_(['risk', 'task'])
        ).first()
        ...
        for m in mitigations:
            if completed_status_id and m.status_id == completed_status_id:
            ...
            if m.due_date and m.due_date < today_date:
                if not completed_status_id or m.status_id != completed_status_id:
                    overdue_mitigations += 1


        mitigation_completion_rate = round(
            completed_mitigations / mitigation_count, 3
        ) if mitigation_count > 0 else 0.0
